...
providerController.insertProviders = async (req, res) => {
    try {
        ...
        return res.status(201).json({message: "Proveedor se ha guardado correctamente"})
    } catch(error){
        console.log("error" + error)
        return res.status(500).json({message: "Internal server error"})
    }
...
providerController.deletedProviders = async (req, res) => {
    try {
...
providerController.updateProviders = async (req, res) => {
    try {
        const {name, phone} = req.body;
        const providerFound = await providerModel.findById(req.params.id);
